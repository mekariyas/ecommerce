...
const Products = () => {
  ...
  const handleDataFetch = async () => {
    try {
      const dataFetched = await instance.get(
        `/admin/getProducts/?page=${searchParams.get("page")}&skip=${searchParams.get("skip") ?? 0}`,
        {
          headers: {
  ...
  const handleDelete = async (product: string) => {
    ...
    try {
      const deleteData = await instance.delete("/admin/deleteProduct", {
        data: { name },
      });
    } catch (error) {
      if (error instanceof AxiosError) {
    ...
    <section className="w-full h-[80vh] md:h-full relative top-16 md:static">
      {products.length > 0 ? (
        <>
          <ul className="w-full flex flex-col items-center md:flex-row md:flex-wrap md:items-between justify-center md:justify-start md:space-x-4 mb-2 md:pl-2 pt-4">
            {products.map((product) => {
              return (
                <li
                  key={product._id}
                  className="w-[80%] md:w-[45%] shadow-2xl  flex flex-col  items-start md:space-x-2 rounded-lg mb-2"
                >
                  <img
                    src={`https://res.cloudinary.com/${cloudName}/image/upload/c_scale,w_260,h_220/q_auto/f_auto/${product.image}`}
                    alt={product.name}
                    className="w-full h-[55%]  md:object-cover rounded-tl-md rounded-tr-md"
                    loading="lazy"
                  />
                  <section className="ml-2">
                    <h1 className="md:text-lg font-semibold w-full">
                      Name: {product.name}
                    </h1>
                    <h2 className="md:text-lg font-semibold w-full">
                      Brand: {product.brand}
                    </h2>
                    <p className="md:text-lg font-semibold w-full">
                    ...
                    <p className="md:text-lg font-semibold w-full text-wrap">
                  ...
                  <section className="w-full flex justify-center items-center gap-2 h-20">
                    <button
                      className="bg-orange-500 w-[20%] h-[70%] rounded-lg text-center flex items-center justify-center text-white cursor-pointer"
                      onClick={() => handleEditPageRoute(product.name)}
                    >
                      <FaRegEdit className="ml-2 w-[30%] h-[75%]" />
                    </button>
                    <button
                      className="bg-red-600 w-[20%] h-[70%] rounded-lg text-center flex items-center justify-center text-white cursor-pointer"
                      onClick={() => handleDelete(product.name)}
                    >
                      <CiTrash className="w-[25%] h-[90%]" />
                    </button>
                  </section>
                </li>
              );
          ...
          <Pagination totalProducts={totalProducts} id={id} />
        </>
      ) : (
        <Loading />
      )}
